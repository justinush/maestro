...
func (e Event) String() string {
	...
	case EventActionRan:
		return fmt.Sprintf("%04d %s step=%q list=%s action=%q type=%q", e.Seq, e.Type, e.StepID, e.ActionList, e.ActionID, e.ActionType)

	case EventTransitionTaken:
		return fmt.Sprintf("%04d %s idx=%d from=%q to=%q", e.Seq, e.Type, e.TransitionIndex, e.FromStepID, e.ToStepID)
